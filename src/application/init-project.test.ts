...
it.effect("InitProject.init writes DEFAULT_CONFIG via ConfigStore", () =>
  Effect.gen(function* () {
...
it.effect("InitProject.init returns ConfigError when writeConfig fails", () =>
  Effect.gen(function* () {
    const result = yield* (yield* InitProject).init().pipe(Effect.result)

...
/** Query-term coverage measured independently for lexical and identifier channels. */
...
/** Scale-independent score-curve evidence for one channel. */
interface ScoreGeometryEvidence {
...
interface PairwiseAgreementEvidence {
...
/** Confidence derived from the dense channel score distribution. */
interface DenseConfidenceEvidence {
...
/** Evidence available for one channel during routing. */
...
/** Complete observable evidence used by the evidence router. */
...
/** Build query-term coverage signals from the indexes used by lexical channels. */
...
const scoreSeparation = (ranking: ChannelRankings[ChannelName]): number => {
  ...
  return normalizedGap(top, reference)
}
...
const scoreGeometry = (ranking: ChannelRankings[ChannelName]): ScoreGeometryEvidence => {
  ...
  const normalized =
    range === 0 ? scores.map(() => 1) : scores.map((score) => clamp((score - last) / range))
...
const buildDenseConfidence = (ranking: ChannelRankings["dense"]): DenseConfidenceEvidence => {
  if (ranking.length === 0)
...
/** Derive observable evidence from one query and its channel rankings. */
...
/** Calculate non-negative per-channel fusion weights from observable evidence. */

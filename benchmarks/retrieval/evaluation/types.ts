...
import type { CorpusManifest, GoldLocation, QueryKind } from "../corpus/manifest.js"
...
/** Compute budget shared by the router search funnel. */
 ...
 * survivors; only the final few candidates are evaluated at full fidelity.
 */
export interface HalvingFunnelStrategy extends RouterSearchBudget {
  readonly kind: "halving-funnel"
  readonly algorithm: string
  readonly spreadSurvivors: 32
  readonly finalists: 256
...
export type RouterSearchStrategy = HalvingFunnelStrategy
...
export const DEFAULT_HALVING_FUNNEL_STRATEGY: HalvingFunnelStrategy = {
  kind: "halving-funnel",
  algorithm: `halton-${FUNNEL_ALGORITHM_PREFIX}`,
...
/** Build the recorded search strategy for one scout sequence. */
export const routerSearchStrategyFor = (
  scoutSequence: ScoutSequenceName,
): RouterSearchStrategy => ({
  ...DEFAULT_HALVING_FUNNEL_STRATEGY,
  algorithm: `${scoutSequence}-${FUNNEL_ALGORITHM_PREFIX}`,
})

/** Shared search-space parameters. */
export const DEFAULT_ROUTER_SEARCH_STRATEGY_PARAMETERS = DEFAULT_HALVING_FUNNEL_STRATEGY
...
export interface QueryMeasurement {
  ...
  readonly size: CorpusManifest["size"]
...
/**
 * Local-stability tie-break recorded for one selected archive candidate: share of its
 * single-coordinate archive neighbours within `STABILITY_EPSILON` of its objective metric, and the
 * widest single-level jump among those plateau neighbours.
 */
export interface SelectionStability {
  readonly metric: QualityMetric
...
export interface RouterSearchTimings {
  ...
  /** Time spent in the halving funnel waves. */
  readonly funnelSearchMs: number
...
export interface RouterSearchDiagnostics {
  ...
  /** Candidates generated by the local Sobol cloud around the final beam (0 when disabled). */
  readonly localCloudCandidates: number
...
export interface EvidenceRouterSearchResult {
  ...
  readonly guardrailsMet: boolean
  readonly promotionStatus: PromotionStatus
  /** Local-stability tie-break metrics of the selected archive candidate. */
...
export interface BenchmarkArtifact {
  ...
  /** Sobol points sampled per elite in the local cloud around the final beam (0 disables). */

...
| 6a    | Done        | +3 new files, 14 modified                                 | Created dtype.ts (EmbeddingDtype, IndexMeta, DtypeMismatchError), vector-decoder.ts (fp32 + stubs), assert-never.ts. Search uses cosine similarity. Embedder L2 normalization removed. serializeVectors now Effect. index-meta.json lifecycle. Added `assertNever` exhaustive check. 163 tests pass. Note: vector-decoder.ts/dtype.ts need tests, Schema audit list created (6 opportunities for Step 9). |

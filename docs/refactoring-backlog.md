...
| 4     | Done        | +2 new files, 1 modified                                  | Extracted PathFilter, vector-math, checkIndexExists/ensureIndexExists, split search into 4 functions. **Fixed bug**: makeOnlyFilter had inverted semantics (negation caused onlyPaths to exclude matching files). PathFilter made internal. Added 13 tests. 162 tests pass.                                                                                                                                                                                                                   |
| 5     | Done        | +1 new file, 3 modified                                   | Extracted search-output.ts (5 functions), clampTopK to validation.ts. Fixed dead export buildContentFields. applyCharBudget uses formatResultMetadata. Note: search-output.ts needs tests (deferred). 162 tests pass.                                                                                                                                                                                                                                                                         |
| 6a    | Done        | +3 new files, 14 modified                                 | Created dtype.ts (EmbeddingDtype, IndexMeta, DtypeMismatchError, VectorDecodeError, VectorEncodeError, UnknownEmbeddingDtypeError), vector-codec.ts (fp32 + stubs). Search uses cosine similarity. Embedder L2 normalization removed. serializeVectors now Effect. index-meta.json lifecycle. Unreachable dtype path uses Effect.die (compile-time guaranteed impossible). 163 tests pass. Note: vector-codec.ts/dtype.ts need tests, Schema audit list created (6 opportunities for Step 9). |
| 6     | Done        | 1 moved, 2 modified                                       | Moved models.ts → services/. Updated CONTEXT.md (Logger→Display, Embedding dtype, index-meta.json). DisplayUpdatePayload kept in ports.ts (correct — moving would invert dependency). 163 tests pass.                                                                                                                                                                                                                                                                                         |
| 7     | Done        | +1 new file, 4 modified                                   | Extracted mergeConfig to lib/config-merge.ts. Removed --force/--verbose flags (2 tests removed). clampPositive in validation.ts. buildIndexOptions simplified. catchAll(reportError) standardized. 161 tests pass.                                                                                                                                                                                                                                                                            |
| 8     | Done        | 1 modified                                                | Added 3 missing error codes (DisplayLogError, UnsupportedFormat, ExtractionFailed). Replaced manual type guards with single formatError using \_tag switch. Context field extraction (model, file, path, stack). 8 new tests. 169 tests pass.                                                                                                                                                                                                                                                 |
| 8.1   | Done        | 2 modified                                                | Made chunkConcurrency and ignoreGitignore required in config schema. Removed 'as number' cast and ?? fallbacks from mergeConfig. Simplified config merging. 169 tests pass.                                                                                                                                                                                                                                                                                                                   |
| 9     | Done        | Multiple files                                            | Converted IndexMeta to Schema. Eliminated typecasts (scanner, Display, fixtures). Consolidated dtype literals into EmbeddingDtypeSchema. CONTEXT.md verified consistent. MODEL_REGISTRY dtypes restored to manually-verified per-model values. 0% dead code, 0% dead exports. 169 tests pass.                                                                                                                                                                                                 |

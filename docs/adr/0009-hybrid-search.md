...
**Why pure functions over ports**: BM25 and Dense scoring are stateless transformations. The only I/O (loading index data) remains in VectorStore. Adding a new retrieval path (e.g. cross-encoder reranker) means adding a scoring function + wiring it into `Effect.all` — no new port/layer.
